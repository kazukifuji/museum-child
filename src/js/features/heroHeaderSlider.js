...
export default {
  ...
  init: function() {
    ...
    this.swiper = new Swiper( this.element, {
      ...
      on: {
        ...
        resize: () => {
          objectFitPolyfill( this.swiper.el.querySelectorAll('.swiper-slide > img') );
        },
        slideChangeTransitionStart: () => {
          const activeSlideImg = this.swiper.slides[ this.swiper.activeIndex ].children[0];
          const previousSlideImg = this.swiper.slides[ this.swiper.previousIndex ].children[0];
          activeSlideImg.style.animation = 'zoom-out 10s cubic-bezier(0.5, 1, 0.89, 1) both';
          setTimeout( () => {
            previousSlideImg.style.animation = '';
          }, this.swiper.params.speed );
        },
      },
      pagination: {
        ...
        renderBullet: function( index, className ) {
          ...
          return '<span class="' + className + '"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="30" height="30" viewBox="0 0 30 30">' +
                   '<path d="' + dValue + '"></path>' +
                 '</svg></span>';
        },
      },
  ...
  play: function() {
    ...
    this.swiper.slides[0].children[0].style.animation = 'zoom-out 10s cubic-bezier(0.5, 1, 0.89, 1) both';
  },
}